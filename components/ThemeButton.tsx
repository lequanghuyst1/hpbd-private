...
const ThemeButton = () => {
  ...
  // Sync checkbox with theme only on initial mount
  useEffect(() => {
    if (mounted && checkboxRef.current) {
      checkboxRef.current.checked = resolvedTheme === "dark";
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [mounted]); // Only sync on mount, not when theme changes

  // Handle theme change with View Transition API animation
  const handleThemeChange = useCallback(
    (newTheme: "light" | "dark") => {
      if (!containerRef.current || isDisabled) return;

      // Disable button for 1 second to prevent spam clicking
      setIsDisabled(true);
      setTimeout(() => {
        setIsDisabled(false);
      }, 1000);

      // Delay 400ms to let button animation (lăn qua lăn lại) run first
      setTimeout(() => {
        // Get container position for animation
    ...
    <div>
      ...
      >
        <input
          ...
          disabled={isDisabled}
          onChange={(e) => {
            // Checkbox toggles automatically, sync theme with it after 400ms delay
            const newTheme = e.target.checked ? "dark" : "light";
            handleThemeChange(newTheme);
          }}
        />
        <div ref={containerRef} className="theme-switch__container">
